...
from .measurement import MeasurementDesign, allocate_counts, covers
from .pauli import (
...
class ExperimentalArchive:
    ...
    def moment_vector(self, setting: np.ndarray, start: int, count: int) -> np.ndarray:
        ...
        if stop > len(outcomes):
            raise ValueError(
                f"Not enough outcomes for setting {tuple(setting)}: need {stop}, have {len(outcomes)}"
            )
...
def estimate_experimental_nonlinear(
    hamiltonian: Hamiltonian,
    design: MeasurementDesign,
    archive: ExperimentalArchive,
    shots: list[int],
    ...
    for total_shots in shots:
        ...
        uncovered_count = 0
        for repeat in range(repeats):
            term_totals = np.zeros(hamiltonian.num_terms, dtype=float)
            term_coverage = np.zeros(hamiltonian.num_terms, dtype=np.int64)
            for setting, allocated_pairs in zip(design.settings, allocated):
                if allocated_pairs == 0:
                    continue
                physical = _physical_setting(setting)
                first_setting = physical[:half]
                second_setting = physical[half:]
                copies = int(np.ceil(np.sqrt(int(allocated_pairs))))
                first_moments = archive.moment_vector(
                    first_setting, repeat * copies, copies
                )
                second_moments = archive.moment_vector(
                    second_setting, repeat * copies, copies
                )
                hit = np.all(
                    (hamiltonian.paulis == 0) | (hamiltonian.paulis == physical), axis=1
                )
                indices = np.flatnonzero(hit)
                first_masks = support_masks(hamiltonian.paulis[indices, :half])
                second_masks = support_masks(hamiltonian.paulis[indices, half:])
                weight = copies * copies
                term_totals[indices] += (
                    first_moments[first_masks] * second_moments[second_masks]
                )
                term_coverage[indices] += weight
            measured = term_coverage > 0
            estimates[repeat] = hamiltonian.offset + float(
                np.sum(
                    hamiltonian.coefficients[measured]
                    * term_totals[measured]
                    / term_coverage[measured]
                )
            )
            uncovered_count = int(np.count_nonzero(~measured))
        output[int(total_shots)] = EstimateBatch(estimates, uncovered_count)
    return output
